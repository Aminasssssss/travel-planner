...
export class ItineraryAnalysis implements OnInit {
  ...
  loadBurnout() {
    this.api.getBurnoutAnalysis(this.itineraryId).subscribe({
      next: (data) => { this.burnout = data; this.cdr.detectChanges(); }
    });
  }
  ...
  getCategoryIcons(categories: any): string {
    const labels: any = {
      'nature': 'Природа', 'history': 'История', 'food': 'Еда',
      'active': 'Активный', 'photo': 'Фото', 'culture': 'Культура',
      'nightlife': 'Ночная жизнь', 'shopping': 'Шопинг',
      'family': 'Семья', 'lifestyle': 'Лайфстайл'
    };
    return Object.entries(categories)
      .map(([cat, count]) => `${labels[cat] || cat}: ${count}`)
      .join(' · ');
  }
