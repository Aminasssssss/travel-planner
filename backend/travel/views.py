...
class DestinationPlacesView(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        if indoor == 'true':
            queryset = queryset.filter(is_indoor=1) 
        elif indoor == 'false':
...
class SavedPlaceListCreateView(generics.ListCreateAPIView):
    ...
    def create(self, request, *args, **kwargs):
        ...
        try:
            ...
            existing = SavedPlace.objects.get(
                user=request.user,
                place_id=place_id
            )
            serializer = self.get_serializer(existing)
...
def weather(request, destination_id):
    try:
        ...
        if response.status_code == 200:
            ...
            if temp < 0:
                recommendation = " Холодно! Возьмите тёплую куртку"
            elif temp < 10:
                recommendation = " Прохладно. Наденьте куртку"
            elif temp < 20:
                recommendation = " Комфортно. Лёгкая куртка"
            elif temp < 30:
                recommendation = " Тепло. Идеально для прогулок"
            else:
...
def burnout_analysis(request, itinerary_id):
    try:
        ...
        if overloaded_days >= 2:
            overall = ' Маршрут перегружен! Рекомендуем сократить количество активностей или увеличить количество дней.'
